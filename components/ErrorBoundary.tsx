  ...
  render() {
    if (this.state.hasError) {
      return this.props.fallback || (
        <div className="flex flex-col items-center justify-center min-h-[30vh] gap-3 p-6">
          ...
          <p className="text-xs text-muted/60 text-center max-w-md">{this.state.error?.message}</p>
          <button onClick={() => window.location.reload()} className="text-xs text-accent hover:underline mt-2">Reload page</button>
        </div>
      );
    }
