      ...
      {showConfirm ? (
        <div className="panel p-4 flex flex-col items-center gap-3 bg-black/80 backdrop-blur-sm">
          ...
          <div className="flex gap-2">
            <Button variant="ghost" className="!py-1.5 !px-3 text-xs" onClick={() => setShowConfirm(false)}>Cancel</Button>
            <Button variant="danger" className="!py-1.5 !px-3 text-xs" onClick={onExit}>Exit Game</Button>
          </div>
        </div>
      ) : (
        <div className="flex gap-2">
...
export default function SessionPage() {
  ...
  return (
    <div className="fixed inset-0 bg-black z-40 select-none" onMouseMove={reveal}>
      <RemoteDesktop className="absolute inset-0" />
      {hud && <HUD />}
      {controls && <GameControls onExit={() => window.location.href = "/home"} />}
      <button
        onClick={() => setHud((v) => !v)}
