...
const STORE: Record<string, { userId: string; username: string; avatar: string; connectedAt: string }> = {};
  ...
  try {
    const tokenRes = await fetch("https://embed.gog.com/token", {
      ...
      body: new URLSearchParams({
        ...
        code,
        redirect_uri: `${new URL(req.url).origin}/api/providers/gog/callback`,
      }),
    });
    ...
    if (tokenData.access_token) {
      ...
      STORE[userId] = { userId, username, avatar, connectedAt: new Date().toISOString() };
