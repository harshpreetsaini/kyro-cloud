...
export class RuntimeManager {
  ...
  async startStreaming() {
    try {
      // Colab can only use the VNC-over-WebSocket tunnel (WebRTC/Selkies require a
      // public signaling server + GPU encoder that aren't available there), so force it.
      const streamingKind = this.providerKind === "colab" ? "vnc" : this.streamingKind;
  ...
  emitAgentEvent(type, payload) {
    ...
    if (["ready", "agent_attached", "agent_disconnected", "desktop_ready", "stream_ready", "vnc_ready", "stats", "system_info", "pong"].includes(type)) {
    ...
    if (type === "ready") {
      ...
      if (this._pingSent) {
        ...
        this.setStats({ latencyMs: rtt, latencySource: "control" });
      }
