...
export class RuntimeManager {
  constructor() {
    ...
    this.apps = {};
    this.installedGames = { games: [], count: 0 };
    // The currently authenticated user on the shared cloud PC (single PC model).
    // Used to attribute installed games + provider links to the right account.
  ...
  launchGame(id) {
    ...
    const payload = {
      id: g.id,
      name: g.name,
      appId: g.providers?.[0]?.appId || "",
      provider: g.providers?.[0]?.type || "steam",
      // executable/arguments/workingDir are legacy catalog fields (usually
      // empty). The agent resolves the real target from its install dir.
  ...
  emitAgentEvent(type, payload) {
    ...
    if (type === "ready") {
      ...
      type === "game.install.progress" ||
      type === "game.install.done" ||
      type === "game.uninstall.done" ||
      type === "game.started" ||
      type === "game.stopped" ||
      type === "game.start_error" ||
      type === "app.launch_result" ||
      type === "app.stop_result" ||
      type === "provider.logout.result" ||
      type === "error" ||
      type === "provider.login.result" ||
      type === "steam.guard" ||
      type === "games.installed" ||
      type === "provider.library" ||
      type === "provider.entitlement"
    ) {
      // Agent-originated client events — forward to every connected client so
      // the frontend store can drive install progress, account linking,
      // Steam Guard prompts, launch results and the installed-games library.
      this.broadcast({ type, payload, ts: Date.now() });
      if (type === "games.installed") {
        this.installedGames = payload || { games: [], count: 0 };
      }
    }
