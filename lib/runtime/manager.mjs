...
export class RuntimeManager {
  ...
  attachVncTunnel(ws) {
    ...
    const onData = (data) => {
      if (this._vncBuffering) {
        ...
        // Cap buffer at 5MB to prevent memory leaks.
        if (this._vncBuffer.length > 200) this._vncBuffer.shift();
      }
    };
  ...
  flushVncBuffer(clientWs) {
    ...
    for (const chunk of this._vncBuffer) {
  ...
  emitAgentEvent(type, payload) {
    ...
    if (type === "ready") {
      ...
      if (payload.netState) {
        this._adaptToNetwork(payload.netState);
      }
    } else if (type === "pong") {
      if (this._pingSent) {
