...
// Reject a pre-upgrade socket with a real HTTP status so proxies report 401
// instead of translating the reset into a 502.
...
export function setupWebSocket(server) {
  ...
  async function handleClient(ws, manager, url) {
    ...
    if (userId != null) {
      ...
      // Backend hook: hydrate this user's durable linked providers so the
      // runtime is attributed to the right account without any re-linking.
      try { globalThis.__onActiveUser?.(userId); } catch {}
    }
