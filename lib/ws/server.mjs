...
export function setupWebSocket(server) {
  ...
  function handleClient(ws, manager) {
    ...
    // Replay persisted linked-provider accounts so the UI shows the linked state
    // on every page and after a refresh (backend is the source of truth).
    try {
      const linked = globalThis.__linkedAccounts || {};
      for (const [provider, info] of Object.entries(linked)) {
        init.push({ type: "provider.login.result", payload: { provider, ok: true, username: info.username }, ts: Date.now() });
      }
    } catch {}
  ...
  function handleAgent(ws, url, manager) {
    ...
    console.log(`[ws] agent connected (${url.searchParams.get("gen") || "?"})`, flush=true);
    ws._role = "agent";
    ...
    try {
      ...
      console.error(`[ws] attachAgent threw: ${ex && ex.stack ? ex.stack : ex}`, flush=true);
    }
    ...
    ws.on("message", (raw) => {
      try {
        ...
        try {
          ...
          console.error(`[ws] emitAgentEvent(${msg.type}) threw: ${ex && ex.stack ? ex.stack : ex}`, flush=true);
        }
      } catch (ex) {
        console.error(`[ws] agent message parse failed: ${ex}`, flush=true);
      }
    });
    ws.on("close", (code, reason) => {
      console.log(`[ws] agent disconnected (code=${code}, reason=${reason ? reason.toString() : ""})`, flush=true);
      manager.detachAgent();
      manager.emitAgentEvent("agent_disconnected", {});
      manager.notify("Runtime agent disconnected.", "error");
    });
    ws.on("error", (err) => {
      console.error(`[ws] agent socket error: ${err && err.message ? err.message : err}`, flush=true);
      manager.detachAgent();
    });
  }
